...
注册中心（design AD-01~04）：为技能 / MCP 连接器 / 知识库 / 专家四类能力实体提供统一 REST 管理面——CRUD、密钥保护、连接器连通性测试、知识库文档上传切块与关键词检索、专家多角色组合；作为 P4 任务挂载与 P8 运行时装配的实体供给方。仅管理实体与字段，不含运行时执行与装配门控。
...
系统 SHALL 提供 `/api/experts` 的 CRUD：`GET /api/experts` 与 `POST /api/experts` 与 `GET /api/experts/<id>` / `PATCH /api/experts/<id>` 与 `DELETE /api/experts/<id>`。字段含 name/description/system_prompt/role/composed_of/enabled；name 必填唯一（重名 400）；role 取值白名单 {ops-sme,general}、默认 general；新建默认 enabled=true、composed_of 为空（单专家）。composed_of 表示该专家的协作子专家 id 列表：写入时元素须为正整数、在专家表中存在、不含自身、同一数组内不重复，否则返回 400。列表类出口返回原始 composed_of 与基础字段；单条读取额外把子专家解析为含 {id,name,role} 的对象列表字段，便于展示组合。PATCH 只应用请求出现的字段；enabled 经 PATCH 启停。删除专家 SHALL 级联清除其任务挂载；若该专家正被其它专家的 composed_of 引用则返回 400（引用完整性），防止悬空组合。

#### Scenario: 组合多专家并可读回子专家明细

- **WHEN** 用户先建专家 A，再新建专家 B 并在 composed_of 中给 A 的 id
- **THEN** 创建 B 成功；GET /api/experts/<B> 返回 composed_of=[A] 且含解析出的子专家对象（A 的 id/name/role）
