...
系统 SHALL 支持 `GET /api/tasks/<id>/files` 返回该任务工作空间目录 `data/spaces/<space>/<task>/` 下现有文件列表（含文件名、大小、mime、路径）；任务不存在返回 404。

#### Scenario: 目录为空返回空列表
- **WHEN** 请求一个尚无文件的已存在任务的文件列表
- **THEN** 返回 200 与空数组（任务工作空间目录存在或按需创建）

#### Scenario: 目录有文件时列出
- **WHEN** 该任务工作空间目录下已存在若干文件后请求列表
- **THEN** 返回 200 与含这些文件元数据的数组

### Requirement: 文件两级落盘与元数据记录
系统 SHALL 提供文件落盘服务：文件按 `data/spaces/<space_id>/<task_id>/` 两级目录物理存储，并用 `FileRecord` 记录元数据（space_id、task_id、filename、path、mime、size）。目录与路径均以数据库 id 生成，杜绝路径穿越。
