# 智能体运行时 API（P8 agentscope-runtime，url_prefix=/api，design D4/D5/D7）。
# 三个端点对应 spec「会话发起与任务模型解析 / 流式对话事件契约与会话生命周期 /
# 危险工具二次确认 / 对话历史持久化与读取」：
...
#   - 流自然结束或连接断开（GeneratorExit）→ finally 里 cancel+join worker、
#     释放注册表 → 该任务可再次发起；MCP 关闭由 worker finally 收敛（driver 责任）。
import json
import uuid
...
def chat(task_id: int):
    ...
    def generate():
        try:
            ...
            # SSE 自然结束或连接断开：cancel+join worker，释放注册表 → 任务可再次发起
            sess.cancel()
            sess.dispose()
